...
test.describe("Preset Application E2E Tests", () => {
	test("should apply a query parameter preset to a tab", async ({ context, extensionId }) => {
	...
	test("should deactivate a preset and remove its parameters", async ({ context, extensionId }) => {
		const testPage = await createTestPage(context, "https://example.com");
		const tabId = await getTabId(context, testPage);
		const popupPage = await openPopupPage(context, extensionId, tabId);
		...
		// Activate
		await presetToggle.check({ force: true });
		await expect(testPage).toHaveURL(/.*deact=val.*/, { timeout: 10_000 });
		...
		await expect(testPage).not.toHaveURL(/.*deact=val.*/, { timeout: 15_000 });
