...
export const List: Component<ListProps> = (props) => {
	return (
		<div class={cn("flex min-h-0 flex-1 flex-col")}>
			{/* Search input */}
			<Input
				type="text"
				...
				class={cn("mb-3 h-9 text-xs")}
				data-testid="list-search-input"
			/>

			{/* No search results state */}
			<Show when={props.filteredPresets.length === 0 && props.searchQuery()}>
			...
			<Show when={props.filteredPresets.length > 0}>
				<ScrollArea class={cn("-mr-1 pr-1")} data-testid="presets-list">
					<div class={cn("space-y-3")}>
						<For each={props.filteredPresets}>
							{(preset) => (
								<Card
									class={cn(
										"border-border/60 hover:border-primary/30 p-4 shadow-sm transition-all"
									)}
