...
import { createTestPage, openPopupPage, getTabId } from "./core/helpers";
...
test.describe("Current Tab Sync E2E Tests", () => {
	test("should show the URL of the current tab", async ({
		context,
		extensionId,
	}) => {
	...
	test("should update when the tab URL changes", async ({
		context,
		extensionId,
	}) => {
	...
	test("should handle multiple tabs and show correct context", async ({
		context,
		extensionId,
	}) => {
