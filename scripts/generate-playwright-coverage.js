...
if (coverageFiles.length === 0) {
	console.warn("No Playwright coverage files found. Run tests with CI_COVERAGE=true first.");
	process.exit(0);
}

console.log(`Generating Playwright coverage report from ${coverageFiles.length} file(s)...`);
...
const coverageReport = new MCR({
	...
	onEnd: async (coverageData) => coverageData,
});
	...
	try {
		...
		if (data.type === "istanbul" && data.data) {
			...
			if (hasFullCoverage(data.data)) {
				...
				console.log(`File ${filePath} has istanbul type but no full coverage maps`);
			}
		} else if (data.data && typeof data.data === "object") {
			if (hasFullCoverage(data.data)) {
				coverageDataToAdd = data.data;
			} else {
				console.log(`File ${filePath} has data object but no full coverage maps`);
			}
		} else if (hasFullCoverage(data)) {
			coverageDataToAdd = data;
		} else {
			console.log(`File ${filePath} has no recognizable coverage data with maps`);
		}
...
if (validCoverageData.length === 0) {
			...
			if (data.type === "istanbul" && data.data && data.data.total) {
				...
				if (total.statements) {
					mergedSummary.total.statements = {
						covered:
							(mergedSummary.total.statements.covered || 0) + (total.statements.covered || 0),
						pct: 0,
						skipped: 0,
	...
	if (fileCount > 0 && mergedSummary.total.lines.total) {
		...
		mergedSummary.total.lines.pct =
			Math.round(
				(mergedSummary.total.lines.covered / mergedSummary.total.lines.total) * 100 * 100
			) / 100;
		mergedSummary.total.statements.pct =
			Math.round(
				(mergedSummary.total.statements.covered / mergedSummary.total.statements.total) * 100 * 100
			) / 100;
		mergedSummary.total.functions.pct =
			Math.round(
				(mergedSummary.total.functions.covered / mergedSummary.total.functions.total) * 100 * 100
			) / 100;
		mergedSummary.total.branches.pct =
			Math.round(
				(mergedSummary.total.branches.covered / mergedSummary.total.branches.total) * 100 * 100
			) / 100;
...
console.log(`📊 View the report at: ${resolve(playwrightCoverageDir, "index.html")}`);
